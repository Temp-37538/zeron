...
impl Render for ShortcutsPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let cycle_order_row = widgets::card_row(&theme, false)
            .child(
                div()
                    .flex_1()
                    .min_w(px(160.0))
            ...
            .child(
                widgets::toggle_switch(
                    &theme,
                    cycle_sessions_recently_used,
                    "cycle-sessions-recently-used",
                )
                .id("cycle-sessions-recently-used-toggle")
                .debug_selector(|| "cycle-sessions-recently-used-toggle".into())
                .tab_index(0)
                .role(gpui::Role::Switch)
                .aria_label("Cycle sessions in recently used order")
                .aria_toggled(if cycle_sessions_recently_used {
                    gpui::Toggled::True
                } else {
                    gpui::Toggled::False
                })
