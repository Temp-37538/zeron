...
impl TurnWire {
    ...
    async fn start_proto(queued: bool, v2: bool) -> Self {
        Self::start_config(queued, v2, true, None, "2.0.3", json!({}), false).await
    }
    ...
    async fn start_modern(queued: bool) -> Self {
        Self::start_config(queued, true, true, None, "2.0.8", json!({}), false).await
    }
                ...
                connections.spawn(async move {
                    ...
                    let (status, body) = if missing_resume {
                        ...
                        match path.as_str() {
                            ...
                            // Pending forms for the `form.created` wire test: one
                            // answerable, one carrying an `external` field the
                            // panel cannot drive (declined with a chip).
                            "/api/session/fixture/form" => ("200 OK", r#"{"data":[{"id":"frm_1","sessionID":"fixture","title":"Ask","fields":[{"key":"note","type":"string","title":"Note","description":"Write something"}]},{"id":"frm_2","sessionID":"fixture","title":"Sign in","fields":[{"key":"login","type":"external","title":"Login","required":true}]}]}"#),
                            _ => ("200 OK", "{}"),
                        }
                    } else {
                        match path.as_str() {
...
#[tokio::test]
async fn v2_detection_resolves_from_a_version_bearing_probe() {
    // The fixture answers a version only on the version-bearing routes; the
    // web-UI catch-alls stay version-less. A completed run proves the V2 wire
